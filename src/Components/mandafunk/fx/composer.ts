...
import { SelectiveUnrealBloomPass } from './shaders/UnrealBloomPass'
...
export class Composer {
    ...
    constructor(renderer: WebGLRenderer, mandaScene: MandaScene, camera: PerspectiveCamera) {
    ...
    updateComposer(config: ConfigType) {
        ...
        if (config.composer?.lens?.show) {
            ...
            this.lensPass.uniforms['cylindricalRatio'].value = config.composer.lens.cylindricalRatio
        }

        if (config.composer?.kaleidoscope?.show) {
            this.kaleiPass.uniforms['sides'].value = 8
            this.composer.addPass(this.kaleiPass)
        }
        if (config.composer?.hue?.show) {
            this.huePass.uniforms['hue'].value = config.composer.hue.hue
            this.huePass.uniforms['saturation'].value = config.composer.hue.saturation
        ...
        if (config.composer?.rgb?.show) {
            this.rgbPass.uniforms['angle'].value = config.composer.rgb.angle * Math.PI
        ...
        if (config.composer?.film?.show) {
            ...
            this.filmPass.uniforms['sIntensity'].value = config.composer.film.sIntensity
            this.filmPass.uniforms['nIntensity'].value = config.composer.film.nIntensity
            this.filmPass.uniforms['grayscale'].value = config.composer.film.grayscale
