...
export const updateImageAnimation = function (scene: Scene, config: ConfigType, time: number) {
    for (let image in config.images) {
            ...
            if (mesh) {
                if (config.images[image].animation.position) {
                    for (let axe in config.images[image].animation.position) {
                        mesh.position[axe] = time * config.images[image].animation.position[axe]
                    }
                }
                if (config.images[image].animation.rotation) {
                    for (let axe in config.images[image].animation.rotation) {
                        mesh.rotation[axe] = time * config.images[image].animation.rotation[axe]
                    }
                }
            }
        }
...
export const updateImageFast = function (name: string, scene: Scene, configImage: ImageType) {
    ...
    if (meshObj && material) {
        ...
        meshObj.rotation.set(configImage.rotationX, configImage.rotationY, configImage.rotationZ)
