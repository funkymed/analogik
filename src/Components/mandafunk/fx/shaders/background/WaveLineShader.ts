...
export class WaveLineShader extends ShaderAbstract {
    constructor() {
        super()
        ...
        float getAmp(float frequency) { return texture(iChannel1, vec2(frequency / 512.0, 0)).x; }
        ...
        void main( void )
        {    
            vec2 uvTrue = (vUv.xy*iResolution.xy) / iResolution.xy;
            ...
            for(float i = 0.0; i < 5.0; i++) {
                ...
                float Y = uv.y + getWeight(squared(i) * 20.0) *
                    (texture(iChannel1, vec2(uvTrue.x, 1)).x - 0.5);
