...
export class FractalShader extends ShaderAbstract {
    constructor() {
        super()
        ...
        this.fshader += `
            ...
            vec3 formula(in vec2 p, in vec2 c)
            {
                ...
                for (int i = 0; i < iters; i++) {
                    ...
                    * smoothstep(0.0, 0.01, abs(p.y)*2.0);
                    ...
                    col += hsv(1.0 - max(p.x, p.y) + t*2.0 + iTime, 2.0-lp+t, r);
            ...
            void main( void ) {
                vec2 p = -1.0 + 2.0 * (vUv*iResolution.xy) / iResolution.xy;
