...
export class BallShader extends ShaderAbstract {
    constructor() {
        super()
        ...
        void main( void ) {
            vec2 q = (vUv*iResolution.xy)/iResolution.xy;
            vec2 p = -1.0+2.0*q;
            p.x *= iResolution.x/iResolution.y;

            // camera
            vec3 ce = vec3( cos( 0.232*iTime) * 10., 6.+3.*cos(0.3*iTime), GRIDSIZE*(iTime/SPEED) );
            vec3 ro = ce;
            vec3 ta = ro + vec3( -sin( 0.232*iTime) * 10., -2.0+cos(0.23*iTime), 10.0 );

            float roll = -0.15*sin(0.5*iTime);
