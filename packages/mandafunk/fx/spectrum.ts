...
/**
 * Creates a gradient canvas used for multi-color spectrum rendering.
 * @param w - Canvas width
 * @param h - Canvas height
 * @param op - Opacity (0 to 1)
 * @returns Canvas element with the gradient, or null on failure
 */
function drawGradiant(
  ...
  if (ctx) {
    ...
    gradient.addColorStop(0.4, `#FFFF00${opacity}`);
...
/**
 * Renders a spectrum analyzer (frequency bars) visualization onto a 2D canvas context.
 * Draws frequency data from the analyser as vertical bars, either in a single color
 * or using a multi-color gradient.
 *
 * @param ctx - The 2D canvas rendering context to draw on
 * @param config - Spectrum configuration (color, bgColor, bars, multiColor, etc.)
 * @param analyser - Web Audio API AnalyserNode providing frequency data
 */
