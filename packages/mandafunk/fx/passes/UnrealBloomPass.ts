...
/**
 * Selective Unreal Bloom post-processing pass.
 * Applies a high-quality bloom effect with optional selective rendering
 * (bloom only on specific layers). Based on the Unreal Engine bloom algorithm.
 */
export class SelectiveUnrealBloomPass extends Pass {
  ...
  private save_materials: { [uuid: string]: Material | Material[] } = {
  ...
  constructor(
    ...
    this.renderTargetSelectedObjects.texture.name =
      "UnrealBloomPass.selectedObjects";
  ...
  render(
    ...
    for (let i = 0; i < this.nMips; i++) {
      ...
      this.separableBlurMaterials[i].uniforms["direction"].value =
        SelectiveUnrealBloomPass.BlurDirectionX;
      ...
      this.separableBlurMaterials[i].uniforms["direction"].value =
        SelectiveUnrealBloomPass.BlurDirectionY;
  ...
  private darkenNonBloom = (obj: Object3D) => {
    if ((obj as Mesh).isMesh && !this.bloom_layer.test(obj.layers)) {
      if (!this.save_materials[obj.uuid])
        this.save_materials[obj.uuid] = (obj as Mesh).material as Material;
      (obj as Mesh).material = this.save_materials["dark_material"] as Material;
    }
  };

  private restoreMaterial = (obj: Object3D) => {
    if (this.save_materials[obj.uuid]) {
      (obj as Mesh).material = this.save_materials[obj.uuid] as Material;
    }
  };
}
