...
ChiptuneJsPlayer.prototype.getLibraryVersion = function () {
  ...
  return `OpenMPT Lib v${libVer >> 24}.${(libVer >> 16) & 0xff}.${
    libVer & 0xffff
  }`;
};
...
ChiptuneJsPlayer.prototype.getCoreVersion = function () {
  ...
  return `OpenMPT Core v${libVer >> 24}.${(libVer >> 16) & 0xff}.${
    (libVer >> 8) & 0xff
  }.${libVer & 0xff}`;
};
...
function percToMillibel(perc) {
  ...
  if (perc === 0) {
    ...
    mB = Math.round(
      (100 * 20 * Math.log(perc * (100 / 80) * 0.01)) / Math.log(10)
    );
  } else if (perc <= 100) {
    // 80..100% = 0..1000 millibel
    // 10 db ~= 316.227766%
    mB = Math.round(
      (100 * 20 * Math.log((100 + ((perc - 80) / 20) * 216.227766) * 0.01)) /
        Math.log(10)
    );
  }
